...
class SelfAttn(nn.Module):
    ...
    def forward(self, x): # [b,t,d]
        ...
        q = q.unflatten(-1, (self.heads,-1)).transpose(1,2) # [b,h,t,d]
...
class SelfAttn(nn.Module):
    ...
    def forward(self, x): # [b,c,h,w]
        b,c,h,w = x.shape
        x = x.flatten(2).transpose(-2,-1) # [b,t,d]
        ...
        q,k,v = self.qkv(x).chunk(3, dim=-1) # [b, r^2, h/r*w/r, dim] # [b*r^2, h/r*w/r, n_heads, d_head]?
        q, k = self.rope(q), self.rope(k)
        q = q.unflatten(-1, (self.heads,-1)).transpose(1,2) # [b,h,t,d]
        k = k.unflatten(-1, (self.heads,-1)).transpose(1,2)
        v = v.unflatten(-1, (self.heads,-1)).transpose(1,2)
